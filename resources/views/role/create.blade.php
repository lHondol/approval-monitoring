...
@section('content')
    @include('shared.appbar', ['backRoute' => 'roleView', 'title' => 'Create New Role'])
    <div class="flex justify-center">
        <div class="ui card !w-[800px] !p-8">
            <form class="ui form" method="post" action="{{ route('roleCreate') }}" enctype="multipart/form-data">
                @csrf
        ...
        $(document).ready(function() {
            $('#permissionsDropdown').dropdown({
                onLabelCreate: function(value, text) {
                    return $('<a class="!p-2 ui green label"></a>').html(text + ' <i class="delete icon"></i>');
                }
            });
        });
    </script>
